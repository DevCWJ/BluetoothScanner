...
namespace CWJ.AccessibleEditor
{
    ...
    public struct ConditionalAttributeData<T> : IConditionalAttEssential where T : _Root_ConditionalAttribute
    {
        ...
        private readonly bool isPlayModeMatched;
        private readonly bool isConstantlyMatched;
        private readonly bool forPredicateComparison;
        private readonly Func<bool> predicate;

        public ConditionalAttributeData(MemberInfo memberInfo, object targetObj/*, out T attribute*/)
        {
            isEnabled = false; predicate = null; forPredicateComparison = false;
            isPlayModeMatched = false; isConstantlyMatched = false;
            ...
            {
                Debug.LogError($"<{targetObj.GetType().Name}>().{memberInfo.Name} 에 중복된 {typeof(T).Name} 선언으로 생긴 오류입니다\n\n" + e.ToString(), targetObj is UnityObject ? targetObj as UnityObject : null);
            }
            if (attribute == null)
            {
                return;
            ...
            if (isEnabled = (attribute != null && !attribute.callSituation.Equals(EPlayMode.Off)))
            {
                isPlayModeMatched = attribute.callSituation.Equals(EPlayMode.Always) 
                    || (Application.isPlaying ? attribute.callSituation.HasFlag(EPlayMode.PlayMode) : attribute.callSituation.HasFlag(EPlayMode.NotPlayMode));
                predicate = attribute.FindPredicate<T>(memberInfo, targetObj);
                isConstantlyMatched = isPlayModeMatched && predicate == null;
        ...
        public (bool isEnabled, bool isMatched, bool isPossitive) GetVariableResult()
        {
            return (isEnabled: isEnabled,
                    isMatched: isEnabled && (isConstantlyMatched || (isPlayModeMatched && predicate != null && predicate.Invoke() == forPredicateComparison)),
    ...
    public abstract class _Root_ConditionalAttribute : PropertyAttribute
    {
        ...
        public Func<bool> FindPredicate<T>(MemberInfo memberInfo, object targetObj) where T : _Root_ConditionalAttribute
        {
            ...
            {
                predicate = null;
                BindingFlags bindingFlags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.GetProperty | BindingFlags.GetField;

                PropertyInfo propertyInfo = targetObjType.GetProperty(predicateName, bindingFlags);
                ...
                if (predicate == null) // Smart Predicate
                {
                    ...
                    {
                        var fieldInfo = targetObjType.GetField(predicateName, bindingFlags);
                        if (fieldInfo != null)
                ...
                {
                    MethodInfo methodInfo = ReflectionUtil.GetMethodInfo<bool>(targetObj, predicateName);
